...
namespace codecrafters_http_server.src.Application.Services.Routes.GET
{
    public class EchoRouteHandler : IHttpRouteHandler
    {
        ...
        public override HttpResponse HandleRoute(HttpRequest request)
        {
            ...
            {
                return new HttpResponse(request.Endpoint.Query, HttpStatusCodes.GetHttpResponseStatus(HttpStatusCodes.OK), HttpContentType.Create(HttpContentType.TextType).Value.ToString(), Encoding.UTF8.GetByteCount(request.Endpoint.Query), "gzip");
            }
